...
function pluszadagolas(receptid) {
	...
	for (let i = 0; i < hossza; i++) {
		let szokoz_index = felsorolas[i].innerText.indexOf(" ");
		let mennyiseg = felsorolas[i].innerText.substring(0, szokoz_index);
		console.log(felsorolas[i].textContent);
		felsorolas[i].innerHTML = `<input type="checkbox" class="rounded-checkbox">${Number(mennyiseg) + Number(mennyiseg * (1 / Number((document.getElementById(`
		$ {
			receptid
		}
		_adag `).innerText))))} ${felsorolas[i].innerText.substring(szokoz_index)}`;
	}
